...
| `TRANSPORT_ORDER_DESTINATION` | празно | `PRENOS` | клијент | Када има вредност, након класичног mobile процеса **Пријем** систем креира транспортни налог за примљену товарну јединицу ка локацији чији је назив уписан у вредност. Ако је празно или не постоји, транспортни налог се не креира. |
...
#### Аутоматски транспорт након класичног пријема

Својство `TRANSPORT_ORDER_DESTINATION` служи као прекидач за аутоматско креирање транспортног налога после пријема робе. Вредност мора бити тачан назив постојеће складишне локације, на пример `PRENOS`. Ако локација не постоји, то је конфигурациона грешка и пријем се прекида грешком `NO_SUCH_STORAGELOCATION`.

Ова логика важи само за класичан mobile процес **Пријем**, односно за ток у ком се товарна јединица креира на пријемној/GoodsIn локацији и не прослеђује се циљна локација или циљна товарна јединица. Не активира се за mobile процес **Пријем и складиштење** (`GR Direct`), јер тај процес већ прослеђује `targetLocation` или `targetUnitLoad` и сам завршава директно смештање робе. Не активира се ни за засебан процес **Складиштење**.

Креирани транспортни налог има тип `TRANSFER`, статус `CREATED` (`50`), подразумевани приоритет `NORMAL` (`50`), клијента преузима са товарне јединице, изворну локацију преузима са тренутне локације товарне јединице, а одредишну локацију из `TRANSPORT_ORDER_DESTINATION`.
